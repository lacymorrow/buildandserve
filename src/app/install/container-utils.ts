...
export class ContainerManager {
	...
	async snapshotFileSystemBefore() {
		if (!this.isReady) {
			throw new Error("Container is not ready");
		}

		this.fileSystemSnapshotBefore = await this.getFileSystemSnapshot();
	}

	async snapshotFileSystemAfter() {
		if (!this.isReady) {
			throw new Error("Container is not ready");
		}

		this.fileSystemSnapshotAfter = await this.getFileSystemSnapshot();
	}

	async getChangedFiles(): Promise<ContainerFile[]> {
		if (!this.isReady) {
			throw new Error("Container is not ready");
		}

		if (this.fileSystemSnapshotBefore.size === 0) {
			throw new Error("No file system snapshot before changes");
		}

		if (this.fileSystemSnapshotAfter.size === 0) {
			throw new Error("No file system snapshot after changes");
		}

		const changedFiles: ContainerFile[] = [];

		for (const [path, content] of this.fileSystemSnapshotAfter.entries()) {
			if (
				!this.fileSystemSnapshotBefore.has(path) ||
				this.fileSystemSnapshotBefore.get(path) !== content
			) {
	...
	private async getFileSystemSnapshot(): Promise<Map<string, string>> {
		const snapshot: Map<string, string> = new Map();
		const fileSystem = await this.getFileSystem();

		for await (const entry of fileSystem) {
			if (entry.isFile()) {
				const content = await this.getFileContent(entry.name);
				snapshot.set(entry.name, content);
			}
		}

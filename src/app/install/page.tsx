...
export default function InstallPage() {
	...
	const startContainerInitialization = async () => {
		...
		try {
			...
			setContainerStatus("preloading-templates");
			setContainerProgress(70);

			// Preload template files to speed up later operations
			try {
				await containerManager.preloadTemplateFiles();
			} catch (preloadError) {
				console.warn("Error preloading templates, continuing with installation:", preloadError);
			}
						...
						<div className="bg-muted/50 p-4 border-b">
							<div className="space-y-2">
								<p className="text-sm font-medium flex items-center">
									...
									{containerStatus === "preloading-templates" && "Loading template files..."}

...
import { NotFoundTerminalBackground } from "./not-found-terminal";
...
export const NotFoundPage = ({ containerClassName, descriptionClassName, statusCode = 404 }: NotFoundPageProps) => {
	return (
		<AppRouterLayout themeProvider={NoOpProvider}>
			<div className="relative h-screen w-screen">
				...
				<div className="container relative z-10 flex h-full w-full flex-col items-center justify-center px-4 sm:px-6">
					<Card className={cn("mx-auto w-full max-w-md bg-background/80 backdrop-blur-sm", containerClassName)}>
						<CardHeader className="items-center text-center">
							...
							<CardDescription className={cn("text-base", descriptionClassName)}>
								<span className="mr-2 rounded-md bg-muted px-2 py-0.5 text-xs font-medium text-muted-foreground">{statusCode}</span>
